...
const CocktailDetail = ({ route }) => {
  ...
  if (!details) {
    return <Text style={styles.loadingText}>Chargement...</Text>; // Ajout d'un indicateur de chargement
  }
    ...
    <ScrollView style={styles.container}>
      ...
      {Object.keys(details).map((key) => {
        if (key.startsWith('strIngredient') && details[key]) {
          const ingredientImage = getIngredientImage(details[key]);
          return (
...
// Fonction pour obtenir des images d'ingrédients
